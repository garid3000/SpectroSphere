...
def exposure(inc):
    ...
    if expo_ind < 0:
        expo_ind = 0
        return
    os.system('v4l2-ctl -d0 -c exposure_absolute={}'.format(expovals[expo_ind]))
    print('v4l2-ctl -d0 -c exposure_absolute={}'.format(expovals[expo_ind]))
    for i in range(10):
    ...
    try:
        while count < 20000:
            ...
            if ret:
                ...
                dBuf_ori[count,:  ] = [i,j,k,r, acc_x, acc_y, acc_z, mag_x, mag_y, mag_z, time.time()-t0, expo_ind]
                count += 1
            if count % 50 == 0:
                print("{:6d} | {:6.2f} {:6.2f} {:6.2f} {:6.2f} | {:6.2f} {:6.2f} {:6.2f} | {:6.2f} {:6.2f} {:6.2f} | {:6.2f}min".format(count, i,j,k,r, acc_x, acc_y, acc_z, mag_x, mag_y, mag_z, (time.time()-t0)/60))
            if (count % 10 == 0) and (count > 0):
                #pass #check exposure here
                m_gray = np.max(np.mean(dBuf_img[count-10:count, :, 110:140], axis = 2), axis = 1)
                m_targ = np.max(np.mean(dBuf_img[count-10:count, :,  10:75 ], axis = 2), axis = 1)
        ...
        np.save('{}/img_el0_{:02d}_{:04d}'.format(ddir, epoch, count), dBuf_img[:count, :, :])
        np.save('{}/ori_el__{:02d}_{:04d}'.format(ddir, epoch, count), dBuf_ori[:count, :])
        print("Finished")
        sys.exit()
