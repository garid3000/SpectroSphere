...
if __name__ == "__main__":
    ...
    s = Sscan('/dev/ttyUSB0' if 'dev' not in cli_args else cli_args['dev'] , baudrate=9600, timeout=0.02)

    def test():
        for _ in range(5):
            s.goto(360, 360, wait=False)
            while True:
                if abs(s.get_pos_deg(1) - 360) + abs(s.get_pos_deg(2) - 360) < 1:
                    break
                print(s.get_pos_deg(1), s.get_pos_deg(2))
            s.goto(-360, -360, wait=False)
            while True:
                if abs(s.get_pos_deg(1) + 360) + abs(s.get_pos_deg(2) + 360) < 1:
                    break
